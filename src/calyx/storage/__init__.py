...
:class:`LedgerConnection` protocol. The engine (:mod:`calyx.core` and
:mod:`calyx.ledger`) speaks only that interface; it never sees
``aiosqlite.Connection`` or ``asyncpg.Connection`` directly.

Data classes (``LedgerRow``, ``LimitRow``, ``CommitOutcome``) live here
rather than in :mod:`calyx.ledger` because they are returned by
...
v0 ships :class:`calyx.storage.sqlite.SqliteBackend` and
:class:`calyx.storage.postgres.PostgresBackend`. New backends can be
