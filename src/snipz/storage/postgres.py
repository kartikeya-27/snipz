...
* :class:`PostgresLedgerConnection` — implements
  :class:`snipz.storage.LedgerConnection` over an ``asyncpg.Connection``.
...
asyncpg is an optional dependency (``pip install snipz[postgres]``).
...
try:
    ...
    _IMPORT_ERROR = ImportError(
        ...
        "Install with: pip install 'snipz[postgres]'"
    )
    _IMPORT_ERROR.__cause__ = _exc


_MIGRATIONS_PACKAGE: Final = "snipz.storage.migrations.postgres"
_LOGGER: Final = logging.getLogger("snipz.storage.postgres")
...
class PostgresLedgerConnection:
    ...
    from :mod:`snipz.storage.sql.postgres`. asyncpg's native type
...
class PostgresBackend:
    ...
    Default (managed pool)::
        ...
        backend = PostgresBackend("postgres://localhost/snipz")
    ...
    def _maybe_warn_pool_pressure(self, pool: Pool) -> None:
        ...
        _LOGGER.warning(
            "Snipz Postgres pool near capacity (%d/%d). "
