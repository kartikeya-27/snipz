# Calyx — Concurrency Scenarios (Phase 0 Paper Validation)
...
T=0ms   A: BEGIN
T=0ms   B: BEGIN
T=1ms   A: SELECT * FROM calyx_limits WHERE (user,u1,month) FOR UPDATE
...
```
calyx_limits: ('user', 'u1', 'month', cap_cents=10000)
calyx_ledger: (empty for this scope)
```
...
```
1. SELECT * FROM calyx_ledger WHERE request_id = $1
...
6. INSERT INTO calyx_ledger (..., request_id = $1)
   On UNIQUE violation:
     ROLLBACK
     SELECT * FROM calyx_ledger WHERE request_id = $1
...
```
calyx_limits: ('user', 'u1', 'month', cap_cents=1000)
calyx_ledger: (empty)
```
...
T=305s    Sweeper runs (every 60s).
          UPDATE calyx_ledger SET state='released', late=TRUE
...
Step 3  SELECT * FROM calyx_limits WHERE (tenant,t1,month) FOR UPDATE
        → acquires; cap=20000
...
`Reservation.commit()` later: `UPDATE calyx_ledger SET state='committed', ... WHERE reservation_id=R` updates both rows atomically.
